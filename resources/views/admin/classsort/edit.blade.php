...
<div class="panel-body">
    ...
    <form  action="/admin/lessonsort/{{ $data->id }}" method="post" enctype="multipart/form-data" class="form-horizontal group-border hover-stripped">
    {{ csrf_field() }}
    {{ method_field('PUT') }}
      <div class="form-group">
        <label class="col-lg-2 col-md-2 col-sm-12 control-label">分类名称</label>
        <div class="col-lg-10 col-md-10">
          <input type="text" class="form-control" value="{{ $data->lsname }}" autofocus="autofocus" name="lsname"></div>
      </div>
      <div class="form-group">
		...
		@foreach($user as $k=>$v)
			<option value="{{ $v->id }}" {{ $data->sid == $v->id}}>{{ $v->lsname }}</option>
		@endforeach
