...
<div class="panel-body">
    ...
    <form  action="/admin/lessonsort" method="post" enctype="multipart/form-data" class="form-horizontal group-border hover-stripped">
    {{ csrf_field() }}
      <div class="form-group">
        <label class="col-lg-2 col-md-2 col-sm-12 control-label">分类名称</label>
        <div class="col-lg-10 col-md-10">
          <input type="text" class="form-control" placeholder="请输入分類名稱" autofocus="autofocus" name="lsname"></div>
      </div>
      <div class="form-group">
        <label class="col-lg-2 col-md-2 col-sm-12 control-label">所属分类</label>
        <div class="col-lg-10 col-md-10">
         <select name="sid" id="id" class="form-control">
			<option value="0">--请选择--</option>
		@foreach($data as $k=>$v)
			<option value="{{ $v->id }}" {{ $id == $v->id ? 'selected' : '' }}>--{{ $v->lsname }}--</option>
		@endforeach
         </select>
          </div>
      </div>
