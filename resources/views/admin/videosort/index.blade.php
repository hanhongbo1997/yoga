  ...
  <div id="datatable_wrapper" class="dataTables_wrapper form-inline" role="grid">
    ...
    <table class="table display dataTable" id="datatable" aria-describedby="datatable_info">
      ...
      @foreach($data as $k=>$v)
       ...
       <a href="/admin/videosort/{{ $v->id }}/edit" class="btn btn-yellow">修改</a>
       <a href="/admin/videosort/create/{{ $v->id }}" class="btn btn-sm btn-primary">添加子类</a>
          <form action="/admin/videosort/{{ $v->id }}" method="post" style="display: inline-block;">
            {{ csrf_field() }}
            {{ method_field('DELETE') }}
